...
\mwh{TODO:}
\begin{itemize}
 \item Move types to the end?
\item Fix T-Var so that it ``locates'' types in the same way that
  $\locof{m}{v}$ locates values. Locatedness  is related to $Q \vdash \tau$ I
  think; probably we should ditch it. I am only seeing it
  used in T-Par, and I don't think it's actually needed there.
\item Need a version of T-Par to handle an empty $Q$; what should be
  the type $\tau$ that is returned?
\item With the T-Var change, how should T-App, T-Abs, and T-Fix
  change? The operational semantics scopes access based on the call
  site. We don't have polymorphism in the type system, so we can't do
  that. How to restrict? I think we just leave the mode the same as
  now.
\item Prepare for the use of polymorphism by generalizing the
  judgments that require $m$ to be the same on the type, i.e., on
  $nat^m$. Think about how $m$ could become a variable, or unions and
  intersections. 
