...
\begin{array}{c}
  ...
  \getat{m}{v} = n
    ...
    {
  Q = m \cap P\\\\
  Q = \emptyset \Rightarrow v = \locof{\emptyset}{0}\\\\
...
(located at $Q = \emptyset$) is returned. \mwh{Fix: The previous is
  not correct. See discussion below.} (The alternative of running 
...
\mwh{TODO:}
The $\kw{par}$ block returning a $\locof{\emptyset}{0}$ when $Q =
\emptyset$ is not right. This is because the block could be returning
a pair, and a pair is not specifically located (same goes with
recursive types). I can see two ways to solve this problem, and I
slightly favor the second:
\begin{itemize}
\item Annotate $\kw{par}$ with the expected type of what's returned,
  and then have the semantics generate the properly-typed shell value
  for it. The tricky thing here is how to interpret the current mode
  when type checking the annotation; if this block of code is within a
  function, its mode might be smaller at run-time than it appears at
  compile-time.
\item Create a special ``crash'' value $\bullet$ which can be given
  any type. Evaluation gets stuck when used as a located value
  (number, sum, function), but just expands when used as a non-located
  one (pair, recursive type); e.g.,
  $\eval{\env}{m}{\eproj{i}{\bullet}}{\bullet}$ but trying to
  evaluate $\ebinop{\bullet}{n}$ is a stuck program. I don't think
  this approach harms soundness, but it might allow some ill-typed programs
  you'd expect to get stuck to not do so.
\end{itemize}
...
$e$ except they are wrapped in a $\epar{\{p\}}{...}$ for each
...
the expression will be skipped (and an inaccessible value
returned). Otherwise it will be run by that single principal.

By inspection of the evaluation rules (Figure~\ref{fig:sem}), we can
...
share can be shared.\footnote{We may need a kind of fresh
  identifier to make sure we are lining up the same redexes, and they
  are not ``accidentally'' the same in appearance.} Since $p$ is doing
...
case into the smaller environments in the MT case. See the Wysteria
paper for approach and inspiration!
...
\item Revisit rules for typechecking functions. What if the function
  ...
  that? 
\item Create a crash value (not easy!)
