...
with mode $m$ (more about subtyping below). Compatibility holds if the
classified-as-$\tau$ value is ``located'' at least some principals
$q \in m$. Compatibility $m \vdash \tau$ is given in
Figure~\ref{fig:aux}. The subtyping judgment is also given in
...
fewer locations than it actually is. The combination of compatibility
and subtyping correspond to the semantic function $\locof{m}{v}$. This
function takes $v$ and ``locates'' it at principals in $m$, filtering
out principals not present in $P$. Rule~\rulelab{T-Sub} can be used to
also invoke subtyping, directly.

Rule~\rulelab{T-Par} runs its expression $e$ in par mode involving
...
\mwh{TODO: Discuss how the above is limiting; e.g., it won't typecheck
...
\subsection{Discussion: Closures and Polymorphism}

\mwh{TODO: Talk about why these rules are pretty restrictive for
  closures}
We want polymorphism on the mode in which a function is
executed. Imagine that we can have a type variable $\beta$ that
represents the mode in which a function is called. Then, the mode in
which its body is checked should be $m \cup \beta$ for some $m$. Here,
$m$ is basically the lower bound. It is the set of the principals that
must be present to avoid failure, as imposed by \rulelab{T-Reveal} or
\rulelab{T-Share}. On the other hand, the mode of variables can be
fluid, and controlled by the closure environment or the caller. For
example, it might be that if you call a function in mode $\{ a,b,c\}$
then the returned result has type $\tnat^{\{a,b\}}$, but if you call
it in mode $\{ a \}$ then the result has type $\tnat^{\{a\}}$. By
introducing a type variable representing the mode, we could generalize
the returned type to something like $\tnat^{\{a,b\} \cap \beta}$, where
$\beta$ is the caller's mode. We'll need to have a logic for deciding
set constraints, e.g., to prove statements like $\beta \cup P \vdash
\beta$ (for all literal sets $P$). We also need to adjust rules like
\rulelab{T-Binop} so that premises like $\tau = \tnat^m_\psi$ are
generalized to $\tau = \tnat^{m'}_\psi$ where $m' \vdash m$ and $m
\vdash m'$. More to do!
