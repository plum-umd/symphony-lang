...
constructs (syntactic sugar) are given below.
...
Many of the language's expression forms are standard, including
...
mode. A number $n$ created in mode $m$ (having type $\tnat^m$ can only
be computed on by principals $p \in m$; other principals do not have
access to it. The same goes for functions and sums. The $\eread$ and
$\ewrite{e}$ expressions read from and write to the console. To be
clear about \emph{which} console, the mode should be a singleton
principal. On the other hand, it's possible that a variable $x$ will
be bound at $p$ but have a value only accessible to $q$ in
it. Principal $p$ can still manipulate a placeholder for $x$ but
should never look at its contents (this is important for the encoding
of wire bundles, see below).

We conclude with the MPC-specific constructs in the language.
...
it makes sense to make these proper syntax in the language, with a
...
We can similarly general the $\kw{reveal}$ construct to work on types
...
\subsection{Future work: First-class principal sets and dependent types}
