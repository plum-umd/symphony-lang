...
A key invariant of the semantics is the following: \emph{Assuming the
  current environment $\sigma$ is compatible with (located at) mode
...
(located at $Q = \emptyset$) is returned.\footnote{The alternative of running
...
Here is an argument in favor of running $e$:
\begin{itemize}
\item Running $e$ could have failures or non-termination. For the
  ...
  at these constructs. Non-termination could happen by an infinite
  $\kw{fix}$. For both failures and non-termination, and code after
  the $\kw{par}$ block will not be reached, whereas for the present
  \rulelab{E-Par} rule, which skips $e$, it will. 
\item Running $e$ has no reasonable analogue in the multi-threaded
  semantics world (see next section). If there are no principals to
  run the code, then ``slicing'' $e$ to run at principals in
  $\emptyset$ will have no effect. This is what justifies the skipping
  behavior. The alternative would be to have every principal in $m$
  run $e$ and fail on any operation that requires principals to be
  present. 
\end{itemize}
\mwh{MORE LATER}
