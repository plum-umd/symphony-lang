...
\item Add text to go with sums (no longer encrypted, directly),
  ...
  \begin{itemize}
    ...
    \item No subtyping at present for recursive types. Should we add
      it?
    \end{itemize}
\item Add encrypted sum as a derived form, encoded as type
  $\tprod{\tnat^m_\psi}{(\tprod{\tau_1}{\tau_2})}$. 
\item Add statically known (non dependent) wire bundles as a derived form, encoded as
  tuples. Will involve adding solo mode.
