...
Soon:
\begin{itemize}
\item I think that $m \vdash \tau$ is backwards. Fix this, and then
  fix the definition of subtyping, which relies on it being
  backwards. I also wonder if \rulelab{M-Sum} etc. are wrong because
  they are not ``deep''. They don't seem to match up with
  $\locof{P}{e}$ but maybe they should?
...
only visible to principals $p \in P$. Since not all valuables need to
be explicitly located (if they are in scope, they implicitly
accessible), we distinguish locatable values $u$ vs. others
$v$. Values for numbers, sums, and closures are standard (closures
...
that it contains an expression $e$, not a value $v$; it represents a
...
Rule~\rulelab{T-Sub} defines subsumption, using subtyping.  The
...
\mwh{I think that $m \vdash \tau$ might be backwards: for values of
  type $\tau$ to be accessible under $m$, the type $\tau$ should have
  more principals in it than $m$, but I think this is doing the opposite.}
