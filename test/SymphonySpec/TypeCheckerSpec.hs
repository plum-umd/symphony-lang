...
spec = do
 ...
 --    UVM.Inr a -> a `shouldBe`  (SecT UVM.Top  (BaseT ℙsT))
    ...
    it "() : intlistexp" $
      let a =  (UVM.AddTop (PowPSE (UVM.frhs [VarPE (UVM.var "A"), VarPE (UVM.var "B")]) ))
          t = RecT (UVM.var "X") (SecT UVM.Top ( (SecT UVM.Top ((BaseT UnitT)))  :+:  (SecT UVM.Top ( (BaseT (ℤT InfIPr)) :×: (VarT (UVM.var "X")) ) )))
          t' = (SecT UVM.Top ( (SecT UVM.Top ((BaseT UnitT)))  :+: (SecT UVM.Top ((BaseT (ℤT InfIPr)) :×: t  ) ) ) )
          expr' =  (AscrE  (nullExp (LE (nullExp (BulE))))  t')
