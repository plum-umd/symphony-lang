...
primType op τs = case (op, tohs τs) of
  ...
  (MinusO,  [     ℕT pr₁, ℕT pr₂ ]) | pr₁ ≡ pr₂ → return $ ℕT pr₁
  (TimesO, [     ℤT pr₁, ℤT pr₂ ]) | pr₁ ≡ pr₂ → return $ ℤT pr₁
  (DivO,   [     ℤT pr₁, ℤT pr₂ ]) | pr₁ ≡ pr₂ → return $ ℤT pr₁
  (DivO,   [     ℕT pr₁, ℕT pr₂ ]) | pr₁ ≡ pr₂ → return $ ℕT pr₁
  ...
  case τ of
    (SecT l' sigma) →
      case sigma of
        ...
        (τₗ :×:  τᵣ)  → (isEmbedable τₗ ) ⩓ (isEmbedable τᵣ )
        (ListT τₜ)  →  (isEmbedable τₜ )
        (ArrT _ τₜ)  →  (isEmbedable τₜ )
        _ → False
    _ → False
...
subtype_loc loctyS loctyT d = case loctyS of
  ...
  -- -------Sub-Fun
  -- t1 m -> t2 <: t1' m -> t2'
  (τ₁₁ :→: (η :* τ₁₂ :* isTL)) → case loctyT of
    (τ₁₁' :→: (η' :* τ₁₂' :* isTL')) → do
        l ← elabEMode $ effectMode η
        l' ← elabEMode $ effectMode η'
        loccondₗ ← (subtype τ₁₁' τ₁₁ d)
        loccondᵣ ← (subtype τ₁₂ τ₁₂' d)
        return ((eq_mode l l') ⩓ loccondₗ ⩓ loccondᵣ ⩓ (isTL ⩔ (not isTL')))
        -- t <: t'
  -- -------Sub-RefRO
  -- ref _ t <: ref RO t'
...
eq_locty locty locty'=
  case locty of
    ...
    (ListT τₜ)  →  case locty' of
      (ListT τₜ') → (eq_type τₜ τₜ')
      _ → return False
    (τ₁₁ :→: (η :* τ₁₂ :* isTL)) → case locty' of
      (τ₁₁' :→: (η' :* τ₁₂' :* isTL')) → do
        ...
        return ((eq_mode l l') ⩓ loccondₗ ⩓ loccondᵣ ⩓ (isTL ≡ isTL'))
    ...
    (RefT (Some loc) τ) →  case locty' of
      ...
      _  → return False
    (ArrT None τ) →  case locty' of
      (ArrT None τ') → (eq_type τ τ')
      _  → return False
    (ArrT (Some loc) τ) → case locty' of
      (ArrT (Some loc') τ') → do
        l ← elabEMode loc
        l' ← elabEMode loc'
        loccond ← (eq_type τ τ')
    ...
    (SecT loc (ListT τₜ)) → do
          ...
          return (\x -> (
            do
              ...
              mt ←  (bindType τ ψₜ)
              mt $ mh $ x))
    _ → typeError "matchType: ⊢ₘ _ ˡ→ _ ; the type τ is not of type (SecT loc (ListT τₜ))" $ frhs
              [ ("τ", pretty τ)
              ]
  EBundleP   → case τ of
