...
generateBristol ∷ CktVal → IM BCktVal
generateBristol = \case
  BaseCV ckt@(Ckt gates out) → case gates ⋕! out of
    BaseG bg → case bg of
      BoolBG b → return $ CompBCV (boolToCir b) null
      NatBG _ n → return $ CompBCV (natToCir $ 𝕟64 n) null
    InputG _ _ → return $ InputBCV out
    PrimG op ws → do
      outT ← wireType ckt out
      cs ← mapM (generateBristol ∘ BaseCV ∘ Ckt gates) ws
      oc ← case op :* outT of
        ...
        NotO :* 𝔹T → invCir ^$ getBitLength ckt
        CondO :* _ → muxCir ^$ getBitLength ckt
        EqO :* _ →
          let w :& _ = ws
          in eqCir ^$ getBitLength $ Ckt gates w
      return $ plugInputs (CompBCV oc null) cs

getBitLength ∷ Ckt → IM ℕ
getBitLength ckt@(Ckt _ out) = do
  wt ← wireType ckt out
  return $ case wt of
    𝔹T → 1
...
printBCir bc@(BCir ins ots gates) =
  let rwm = makeReverseWireMap bc
      wgs = show𝕊 (count gates) ⧺ " " ⧺ show𝕊 (count gates + sum ins)
      ins' = fold (show𝕊 $ count ins) (\i acc → acc ⧺ " " ⧺ show𝕊 i) ins
      ots' = fold (show𝕊 $ count ots) (\o acc → acc ⧺ " " ⧺ show𝕊 o) ots
