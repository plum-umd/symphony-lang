module Allyn.Interpreter where
...
parseOptionsAllyn ∷ IO (Options ∧ 𝐿 𝕊)
parseOptionsAllyn = do
  as ← iargs
  let fs :* nos :* ems = parseOptions (usageInfoTop ⧺ usageInfoRun) as
  eachOn ems out
  os ← compose fs ^$ options₀
  when (optVersion os) $ do
    out $ "allyn version " ⧺ allyn_VERSION
  when (optVersion os ⩓ optHelp os) $ do
    out ""
  when (optHelp os) $ do
    out "Usage: allyn [<command>] [<arguments>] [<target>]"
    out ""
    out $ optUsageInfo "allyn [arguments]" usageInfoTop
    out $ optUsageInfo "allyn run [arguments] <file>" usageInfoRun
    out $ optUsageInfo "allyn example [arguments] <name>"  usageInfoExample
    out $ optUsageInfo "allyn test [arguments]" usageInfoTest
  return $ os :* nos
