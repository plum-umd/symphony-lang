module Allyn.Interpreter.NetIO where
...
foreign import ccall unsafe "empc.h netio_create" netio_create ∷ CString → HS.Int → HS.Bool → IO (Ptr NetIOStruct)
foreign import ccall unsafe "empc.h netio_send" netio_send ∷ Ptr NetIOStruct → Ptr a → HS.Int → IO ()
foreign import ccall unsafe "empc.h netio_recv" netio_recv ∷ Ptr NetIOStruct → Ptr a → HS.Int → IO ()
foreign import ccall unsafe "empc.h netio_flush" netio_flush ∷ Ptr NetIOStruct → IO ()
foreign import ccall unsafe "empc.h &netio_destroy" netio_destroy ∷ FinalizerPtr NetIOStruct
